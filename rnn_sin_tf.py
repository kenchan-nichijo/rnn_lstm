...
if __name__ == '__main__':
    ...
    x = np.array(x).reshape(-1, maxlen, 1) #shape:(176,25,1)
    t = np.array(t).reshape(-1, 1) #shape:(176,1)
    ...
    '''
    # sin波の予測
    sin = toy_problem(T, ampl=0.) #ノイズなしのsin波
    gen = [None for i in range(maxlen)] #予測値を時系列で保持するためのリスト

    z = x[:1] #予測用に元データの最初の部分のみ用いる

    #逐次的に予測値を求める
    for i in range(length_of_sequences - maxlen):
        #preds = model.predict(z[-1:])
        preds = model(z[-1:]) #次の一点を予測
